...
class RLottie {
    ...
    pause(containerId?: string) {
        if (containerId) {
            ...
            const areAllContainersPaused = Array.from(
                this.containers.values()
            ).every(({ isPaused }) => isPaused);
    ...
    private addContainer(
        ...
        if (container instanceof HTMLDivElement) {
            ...
            if (!size) {
                ...
                if (!size) {
                    throw new Error(
                        "[RLottie] Failed to detect width from container"
                    );
                }
            }
    ...
    private doPlay() {
        ...
        animate(() => {
            ...
            const delta = Math.min(
                1,
                (this.direction * this.speed) / currentSpeed
            );
            const expectedNextFrameIndex = Math.round(
            ...
            if (
                ...
                (frameIndex === this.stopFrameIndex ||
                    (delta > 0 &&
                        expectedNextFrameIndex > this.stopFrameIndex) ||
...
Object.defineProperty(window, "RLottie", { value: RLottie });
